...
面板使用便于记忆的固定登录页 `/login/`。2026-09-18 已将 3x-ui 的面板基础路径明确设为该路径；访问域名根目录得到 404 是预期行为。订阅链接仍包含随机路径与每个用户独有的 Sub ID。订阅链接是钥匙的一部分，不能写进公开 Git 仓库。
...
| 面板域名打开但不能登录 | 地址是否为 `/login/` 与账号 | 访问了根目录，或账号密码不一致 |
