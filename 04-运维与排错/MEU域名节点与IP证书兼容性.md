...
已将这两个入站的分享地址同步为公网 IP，并重启 x-ui。再次验证订阅响应后，五条节点中 VLESS WebSocket 与 Hysteria2 的地址和 SNI 均为公网 IP。订阅 URL、UUID 和认证值只保留在私有记录中。

随后继续检查 3x-ui 的订阅表单，发现“订阅内容加密（`subEncrypt`）”仍开启。它实际输出的是一整段 Base64 文本；虽然这在一些客户端可用，但用户的 v2rayN 不能稳定识别。已关闭该开关，使订阅直接返回一行一条的 `vless://`、`ss://`、`vmess://`、`hysteria2://` 标准 URI。公网 HTTPS 请求返回 `200`，证书校验通过，响应恰为五条 URI。完整表单、作用和验收方法见 [[3x-ui订阅表单核对清单]]。

现在在 v2rayN 中对原订阅执行“更新订阅”，应取得这份链接列表；如果旧域名静态节点仍留在列表中，需要由使用者自行删除或放入另一个分组，避免把旧节点误当成更新后的订阅节点。订阅机制的基础说明见 [[UUID密码与订阅链接]]。
